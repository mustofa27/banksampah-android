...
public class CartRepository extends BaseRepository {
    ...
    private MutableLiveData<ArrayList<Cart>> cartListMutableLiveData;

    private CartRepository(BaseNetwork baseNetwork, SharedPreferenceHelper sharedPreferenceHelper, VMRepoInterface vmRepoInterface, AppDatabase db) {
        super(baseNetwork, sharedPreferenceHelper, vmRepoInterface, db);
        cartListMutableLiveData = new MutableLiveData<>();
    }

    public static CartRepository getInstance(BaseNetwork baseNetwork, SharedPreferenceHelper sharedPreferenceHelper, VMRepoInterface vmRepoInterface, AppDatabase db) {
        if (instance == null) {
            instance = new CartRepository(baseNetwork, sharedPreferenceHelper, vmRepoInterface, db);
        } else{
    ...
    public LiveData<ArrayList<Cart>> getData(){
        dataSource.Connect(ConnectionHandler.post_method, "cart/my", null, new NetworkCallback() {
            @Override
            ...
            public void onSuccess(Result result) {
                Cart[] carts = dataSource.getGson().fromJson(((Result.Success) result).getData().toString(), Cart[].class);
                ArrayList<Cart> cartArrayList = new ArrayList<>();
                cartArrayList.addAll(Arrays.asList(carts));
                cartListMutableLiveData.setValue(cartArrayList);
    ...
    public void addCart(int product_id){
        ...
        try {
            ...
            dataSource.Connect(ConnectionHandler.post_method, "cart", jsonObject, new NetworkCallback() {
                @Override
                public void onFinish() {
