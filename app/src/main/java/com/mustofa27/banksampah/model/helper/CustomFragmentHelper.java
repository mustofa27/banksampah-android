...
public class CustomFragmentHelper extends Fragment {
    ...
    protected Uri getResizedImage(Context context, Uri mCurrentPhotoPath, String name) throws IOException {
    ...
    protected long getTimePassed(String date, int type) {
        String regex = date.contains("T") ? "T" : " ";
        String[] tmp = date.split(regex);
        String[] tgl = tmp[0].split("-");
        Calendar temp = Calendar.getInstance();
        ...
        temp.set(Calendar.MONTH, Integer.valueOf(tgl[1]) - 1);
        temp.set(Calendar.YEAR, Integer.valueOf(tgl[0]));
        long selisih = now.getTimeInMillis() - temp.getTimeInMillis();
        if(type == Calendar.SECOND){
            ...
            return selisih/((long)1000*60*60*24*30);
        } else {
            return selisih/((long)1000*60*60*24*365);
        }
    }
    ...
    protected void selectFile(Activity activity, int reqCode){
        ...
        startActivityForResult(Intent.createChooser(intent, "Select a File to Upload"), reqCode);
    }

    protected void selectAudio(Activity activity, int reqCode){
        Intent intent = new Intent(Intent.ACTION_GET_CONTENT);
    ...
    protected String getTimeSelesai(String jamMulai, int durasi) {
        ...
        calendar.set(Calendar.MINUTE, Integer.valueOf(jamPisah[1]));
        calendar.setTimeInMillis(calendar.getTimeInMillis() + (long)durasi * 60 * 60 * 1000);
        String jam = (calendar.get(Calendar.HOUR_OF_DAY) < 10 ? "0" : "") + calendar.get(Calendar.HOUR_OF_DAY) + (calendar.get(Calendar.MINUTE) < 10 ? ":0" : ":") + calendar.get(Calendar.MINUTE) + ":00";
        return jam;
    }

    protected String getTimeConsul() {
        Calendar calendar = Calendar.getInstance();
        String jam = calendar.get(Calendar.HOUR_OF_DAY) < 10 ? "0" : "" + calendar.get(Calendar.HOUR_OF_DAY) + (calendar.get(Calendar.MINUTE) < 10 ? ":0" : ":") + calendar.get(Calendar.MINUTE);
        calendar.setTimeInMillis(calendar.getTimeInMillis() + 60 * 60 *1000);
        jam = jam + " - " + (calendar.get(Calendar.HOUR_OF_DAY) < 10 ? "0" : "") + calendar.get(Calendar.HOUR_OF_DAY) + (calendar.get(Calendar.MINUTE) < 10 ? ":0" : ":") + calendar.get(Calendar.MINUTE);
        return jam;
    }
}
